...
import static org.apache.wave.server.model.testing.ExtraAsserts.assertStructureEquivalent;

import org.apache.wave.server.model.conversation.Conversation.Anchor;
import org.apache.wave.server.model.conversation.testing.BlipTestUtils;
import org.apache.wave.server.model.document.MutableDocument;
import org.apache.wave.server.model.document.MutableDocument.Action;
...
import org.apache.wave.server.model.testing.ExtraAsserts;
...
public class WaveletBasedConversationTest extends ConversationTestBase {
  ...
  public void testDeleteBlipNoRepliesUpdatesManifest() {
    ...
    blip.delete();
    assertManifestXml("");
    ExtraAsserts.assertStructureEquivalent(XmlStringBuilder.createEmpty(), blip.getContent());
    assertMirrorConversationEquivalent();
  }
  ...
  public void testDeleteBlipWithInlineReplyUpdatesManifest() {
    ...
    assertManifestXml("");
    ExtraAsserts.assertStructureEquivalent(XmlStringBuilder.createEmpty(), blip.getContent());
    ExtraAsserts.assertStructureEquivalent(XmlStringBuilder.createEmpty(), replyBlip.getContent());
    assertMirrorConversationEquivalent();
  }
  ...
  private void assertManifestXml(final String expected) {
    manifestDoc.with(new Action() {
      ...
      public <N, E extends N, T extends N> void exec(MutableDocument<N, E, T> doc) {
        ...
        ExtraAsserts.assertStructureEquivalent(exp.wrap("conversation"), doc);
      }
    });
  }
