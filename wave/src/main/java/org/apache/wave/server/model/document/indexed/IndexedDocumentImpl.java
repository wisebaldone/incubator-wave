...
import org.apache.wave.server.model.document.AnnotationCursor;
import org.apache.wave.server.model.document.AnnotationInterval;
import org.apache.wave.server.model.document.RangedAnnotation;
import org.apache.wave.server.model.document.indexed.RawAnnotationSet.AnnotationEndEvent;
import org.apache.wave.server.model.document.indexed.RawAnnotationSet.AnnotationEvent;
import org.apache.wave.server.model.document.indexed.RawAnnotationSet.AnnotationStartEvent;
import org.apache.wave.server.model.document.operation.Attributes;
import org.apache.wave.server.model.document.operation.Automatons;
...
import org.apache.wave.server.model.document.operation.Nindo.NindoCursor;
import org.apache.wave.server.model.document.operation.NindoValidator;
import org.apache.wave.server.model.document.operation.algorithm.AnnotationsNormalizer;
import org.apache.wave.server.model.document.operation.algorithm.Composer;
import org.apache.wave.server.model.document.operation.automaton.AutomatonDocument;
import org.apache.wave.server.model.document.operation.automaton.DocOpAutomaton.ViolationCollector;
import org.apache.wave.server.model.document.operation.automaton.DocumentSchema;
import org.apache.wave.server.model.document.operation.impl.AttributesImpl;
import org.apache.wave.server.model.document.operation.impl.AttributesUpdateImpl;
...
import org.apache.wave.server.model.document.operation.impl.DocOpValidator;
import org.apache.wave.server.model.document.operation.impl.UncheckedDocOpBuffer;
import org.apache.wave.server.model.document.raw.RawDocument;
import org.apache.wave.server.model.document.util.AnnotationIntervalImpl;
import org.apache.wave.server.model.document.util.Annotations;
import org.apache.wave.server.model.document.util.DocOpScrub;
import org.apache.wave.server.model.document.util.EmptyDocument;
import org.apache.wave.server.model.document.util.Point;
import org.apache.wave.server.model.document.util.RangedAnnotationImpl;
import org.apache.wave.server.model.document.util.XmlStringBuilder;
import org.apache.wave.server.model.operation.OpCursorException;
import org.apache.wave.server.model.operation.OperationException;
import org.apache.wave.server.model.util.Box;
import org.apache.wave.server.model.util.CollectionUtils;
import org.apache.wave.server.model.util.EvaluableOffsetList;
import org.apache.wave.server.model.util.OffsetList.Container;
import org.apache.wave.server.model.util.OffsetList.LocationAction;
...
public class IndexedDocumentImpl<N, E extends N, T extends N, V>
   ...
   */
  private final LocationAction<N, Point<N>> pointFinder
      = new LocationAction<N, Point<N>>() {

    public Point<N> performAction(Container<N> container, int offset) {
   ...
   * the location.
   */
  private final LocationAction<N,Void> locationUpdater =
      new LocationAction<N, Void>() {

    public Void performAction(Container<N> container, int offset) {
   ...
   */
  private Container<N> currentContainer;
  ...
  private void indexElement(E element) {
    Container<N> sentinel = offsetList.sentinel();
  ...
  public int getLocation(N node) {
    ...
    Container<N> indexingContainer = substrate.getIndexingContainer(node);
  ...
  private int getLastLocationIn(N node) {
    ...
    if (lastChild != null) {
      if (substrate.asElement(lastChild) != null) {
        ...
        Container<N> indexingContainer = substrate.getIndexingContainer(lastChild);
...
//      Map<String, String> oldAttributes = substrate.getAttributes(node);
  ...
  public class NonInvertibleCursor implements NindoCursor {
    ...
    private List<AnnotationEvent> deleteAnnotations(int size) {
      ...
      deletionValues.each(new ProcV<String>() {
        @Override
        public void apply(String key, String value) {
      ...
      for (AnnotationInterval<String> i : annotationIntervals(start, end, null)) {
        ...
        deletionInherit.each(new ProcV<String>() {
          @Override
          public void apply(String key, String value) {
            events.add(
        ...
        i.annotations().each(new ProcV<String>() {
          @Override
          public void apply(String key, String value) {
  ...
  private void deleteCurrentContainer() {
    Container<N> nextContainer = currentContainer.getNextContainer();
   ...
   */
  private E getParentOf(Container<N> container) {
    N domNode = container.getValue();
    if (domNode != null) {
   ...
   */
  private void insertBefore(Container<N> container, N domNode, int size) {
  ...
  public T mergeText(T secondSibling) {
    ...
    if (mergedNode != null) {
      Container<N> previous = currentContainer.getPreviousContainer();
  ...
  public void forEachAnnotationAt(int location,
      final ProcV<String> callback) {
    annotations.forEachAnnotationAt(location, new ProcV<Object>() {
      @Override
      public void apply(String key, Object value) {
        if (!Annotations.isLocal(key)) {
          assert value == null || value instanceof String;
